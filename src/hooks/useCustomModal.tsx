...
function Popup() {
    ...
    const fire: (message: string, confirmButtonText?: string, cancelButtonText?: string) => Promise<boolean> = (message, confirmButtonText, cancelButtonText) => {
        setShowPopup(true);
        setMessage(message);
        if(confirmButtonText)setConfirmButtonText(confirmButtonText)
        if(cancelButtonText)setCancelButtonText(cancelButtonText);
