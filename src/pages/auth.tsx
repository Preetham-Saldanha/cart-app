    ...
    return <div className="relative w-full h-full min-h-screen  bg-[url(/images/bruno-kelzer-LvySG1hvuzI-unsplash.jpg)] bg-cover bg-center bg-no-repeat">
        <div className="relative w-full h-full min-h-screen bg-black lg:bg-opacity-60">
            <div className="flex justify-center">
                <div className="bg-black bg-opacity-70 p-16 self-center mt-28 rounded-md lg:w-2/5">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <button onClick={() => signIn('google', { callbackUrl: "/" })} className="w-full  mt-6 rounded-md bg-zinc-600 text-zinc-300 hover:bg-zinc-500 hover:text-white transition text-xl font-semibold" ><div className="py-3"><FcGoogle size={30} className="inline mr-1" />Google</div></button>
                        ...
                        {/* common */}
                        <p className="text-zinc-400 text-lg "> {variant === 'login' ? "First time using cart-it-off?" : "Already have an account?"}<span className="text-pink-500 font-semibold hover:underline cursor-pointer" onClick={toggleVariant}>{variant === 'login' ? " Create an account" : " Log in"}</span></p>
                    </div>
                </div>
            </div>
        </div>
