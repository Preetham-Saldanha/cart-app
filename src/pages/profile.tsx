...
function CityInput(props: { name: string, type: string, label: string }) {
    ...
    const [initialTyping, setInitialTyping] = useState<boolean>(false);
    const { value } = field;
    ...
    return (
        <>
            <div className='flex flex-col text-xl gap-1 mb-2 md:w-[45%] relative'>
                ...
                {meta.error && meta.touched && <div className='font-semibold text-base text-pink-700'>{meta.error}</div>}
...
const profilepage = () => {
    ...
    const [initialValues, setInitialValues] = useState({ addressLine1: '', addressLine2: '', city: '', state: '', postalCode: '', landmark: '' })
    useEffect(() => {
        if (user?.id) fetcher(`/api/address?userId=${user.id}`).then(res => {
            console.log(res)
            ...
            <Layout>
               ...
               {!loading && <Formik
                    ...
                    validationSchema={Yup.object({
                        ...
                        landmark: Yup.string().max(150, 'Must be 150 characters or less')
                    })}
                    onSubmit={(values, { setSubmitting }) => {
                        console.log(values)
                        axios.post("/api/address", { ...values, email: user.email, userId: user.id }).then(res => console.log(res))
                    }}
                >
                    <Form className='flex flex-col lg:w-1/2  md:w-2/3 w-[90%] rounded-md flex-wrap m-auto bg-white p-6 top-6 relative'>
                        <div className='flex flex-col text-xl gap-1 mb-2'>
                            ...
                            <ErrorMessage name="addressLine1" className='font-semibold text-base text-pink-700' component="div" />
                        </div>
                        ...
                        <div className='flex flex-col text-xl gap-1 mb-2'>
                            ...
                            <ErrorMessage name="addressLine2" className='font-semibold text-base text-pink-700' component="div" />
                        </div>
                        ...
                        <div className='flex md:flex-row flex-col flex-wrap justify-between'>
                            ...
                            <div className='flex flex-col text-xl gap-1 mb-2 md:w-[45%]'>
                                ...
                                <ErrorMessage name='state' className='font-semibold text-base text-pink-700' component="div" />
                            </div>
                            <div className='flex flex-col text-xl gap-1 mb-2 md:w-[45%]'>
                                ...
                                <ErrorMessage name='postalCode' className='font-semibold text-base text-pink-700' component="div" />
                            </div>

                            <div className='flex flex-col text-xl gap-1 mb-2 md:w-[45%]'>
                                <label htmlFor="landmark" className='font-medium'>Landmark</label>
                                <Field name="landmark" type="text" className="profile_form_field" />
                                <ErrorMessage name='landmark' className='font-semibold text-base text-pink-700' component="div" />
                            </div>
